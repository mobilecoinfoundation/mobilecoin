...
`main` is the primary development branch in `mobilecoin.git`.
...
* We branch for release e.g. `release/v2.0` of off `main`.
...
* Any changes that need to be in `main` and release (which is most bug fixes)
  get cherry-picked.
...
* Changes are sometimes cherry-picked from release to `main` in reverse order
  ...
  yet forwards into `main`, which is very confusing for developers and reviewers because
  seemingly unrelated changes appear in the diff.
* Sometimes changes in release are forgotten about and never cherry-picked to
  `main`.  This leads to regressions in the next release. Because we never merge
  the release branch back to `main`, and all the cherry-picks have different
  hashes from their originals, it's very hard to ever be sure that we got all
  the changes.
