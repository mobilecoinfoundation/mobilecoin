...
// Using Hc128Rng because it was (at one point) the StdRng of rand crate, up to version 0.6
// We don't want to depend on rand because it depends on std, and specifically, its
// implementation of ThreadRng depends on rust stdlib thread_local! macro
...
// Get the thread local instance, initializing it if that hasn't happened yet in this thread
...
/// To get rand_core::OsRng, we must turn on `rand_core/getrandom` cargo feature,
/// but this appears to turns on `libc/std`.
/// There is therefore no way to get rand_core::OsRng without turning on the standard library.
/// Fortunately this body of code is trivial
///
/// This may just be a versioning issue? it may be that if rand_core upgrades the
/// version of getrandom that they rely on, we will be able to use rand_core/getrandom
/// without getting std, and then wouldn't have to carry this.
///
