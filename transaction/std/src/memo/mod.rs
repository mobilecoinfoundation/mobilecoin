...
mod tests {
    ...
    #[test] fn test_java_memo() {
        ...
        let tx_public_key_bytes : [u8; 32] = hex::decode(&"c235c13c4dedd808e95f428036716d52561fad7f51ce675f4d4c9c1fa1ea2165").unwrap().try_into().unwrap();
        let tx_public_key = CompressedRistrettoPublic::from(&tx_public_key_bytes);

        let alice_cred = SenderMemoCredential::from(&alice);
        let memo  =
            AuthenticatedSenderMemo::new(&alice_cred, bob.default_subaddress().view_public_key(), &tx_public_key);
