...
fn test_validate_outputs_are_sorted() {
    for block_version in BlockVersion::iterator() {
        ...
        output_a.public_key = CompressedRistrettoPublic::try_from(&[1u8; 32]).expect("Could not construct key");

        let mut output_b = output_a.clone();
        output_b.public_key = CompressedRistrettoPublic::try_from(&[2u8; 32]).expect("Could not construct key");
