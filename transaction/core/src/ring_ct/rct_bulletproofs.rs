...
/// A structure that contains all the data required to sign a transaction that does not rely on having
/// the spend private key.
/// Exposing this allows implementing various offline/hardware signing solutions. The idea is that the data
/// in this object can be generated by only having the view private key for a set of input TxOuts, and it
/// can then be moved to a separate machine/service that then takes it + the spend private key and uses that
/// to generate a fully signed transaction.
/// See the implementation of `sign_with_balance_check` for an example of how this is used.
