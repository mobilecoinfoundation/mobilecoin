...
use mc_transaction_extra::{verify_tx_summary, UnsignedTx};
...
// right now
fn get_current_max_size_transaction(rng: &mut impl CryptoRngCore) -> (UnsignedTx, AccountKey) {
...
// right now
fn get_current_min_size_transaction(rng: &mut impl CryptoRngCore) -> (UnsignedTx, AccountKey) {
    let sender = AccountKey::random_with_fog(rng);
    let recipient = AccountKey::random_with_fog(rng);
...
fn test_max_size_tx_summary_verification() {
    ...
    let (unsigned_tx, sender) = get_current_max_size_transaction(&mut rng);
    ...
    let (mlsag_signing_digest, _report) = verify_tx_summary(
...
fn test_max_size_tx_payload_sizes() {
    ...
    let (unsigned_tx, _sender) = get_current_max_size_transaction(&mut rng);
...
fn test_min_size_tx_payload_sizes() {
    ...
    let (unsigned_tx, _sender) = get_current_min_size_transaction(&mut rng);
    let (signing_data, tx_summary, tx_summary_unblinding_data, _extended_message_digest) =
        unsigned_tx.get_signing_data(&mut rng).unwrap();
    let signature_rct = signing_data
        .sign(&unsigned_tx.rings, &NoKeysRingSigner {}, &mut rng)
