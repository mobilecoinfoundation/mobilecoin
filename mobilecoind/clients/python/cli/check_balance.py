...
if __name__ == '__main__':
    ...
    parser.add_argument('--first_block', help='(optional) first ledger block to scan', nargs='?', const=0, type=int)
    args = parser.parse_args()

    # create a monitor
    entropy = args.key
    account_key = mobilecoind.get_account_key(bytes.fromhex(entropy))
    monitor_id = mobilecoind.add_monitor(account_key, first_subaddress=args.subaddress, first_block=args.first_block).hex()

    # Wait for the monitor to process the complete ledger (this also downloads the complete ledger)
    (monitor_is_behind, next_block, remote_count, blocks_per_second) = mobilecoind.wait_for_monitor(monitor_id_hex)
    ...
    balance_picoMOB = mobilecoind.get_balance(bytes.fromhex(monitor_id), args.subaddress)

    # print account information
    print("\n")
    print("    {:<18}{}".format("Master Key:", entropy))
    print("    {:<18}{}pMOB".format("Balance:", balance_picoMOB))
    print("    {:<18}{}MOB".format(" ", mobilecoind.display_as_MOB(balance_picoMOB)))
    print("\n")
