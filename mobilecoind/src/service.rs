...
> {
    ...
    watcher_db: Option<WatcherDB>,
    network_state: Arc<RwLock<PollingNetworkState<T>>>,
    active_passwords: HashMap<MonitorId, Vec<u8>>,
    monitor_passwords_sender: crossbeam_channel::Sender<MonitorPwMessage>,
    logger: Logger,
}
    ...
    > Clone for ServiceApi<T, FPR>
{
    fn clone(&self) -> Self {
        Self {
            ...
            active_passwords: self.active_passwords.clone(),
            monitor_passwords_sender: self.monitor_passwords_sender.clone(),
    ...
    > ServiceApi<T, FPR>
{
    ...
    fn remove_monitor_impl(
        ...
        self.mobilecoind_db
            .remove_monitor(&monitor_id, password_hash.as_ref())
            .map_err(|err| {
    ...
    fn generate_optimization_tx_impl(
        ...
        // FIXME: Add passwords here
        // Generate optimization tx.
        let tx_proposal = self
            .transactions_manager
            .generate_optimization_tx(&monitor_id, request.subaddress, None)
            .map_err(|err| {
                rpc_internal_error(
...
mod test {
    ...
    fn test_add_monitor_impl(logger: Logger) {
        ...
        let expected_monitor_id = MonitorId::new(account_key, DEFAULT_SUBADDRESS_INDEX, 1, 0);
    ...
    fn test_remove_monitor_impl(logger: Logger) {
        ...
        let monitor_ids: Vec<MonitorId> = (0..monitors_to_add)
            .map(|_i| {
                ...
                .unwrap();
                let monitor_id =
                    MonitorId::new(account_key.clone(), DEFAULT_SUBADDRESS_INDEX, 1, 0);
                mobilecoind_db
                    .add_monitor(&monitor_id, &account_key, &data)
                    .unwrap();
                monitor_id
    ...
    fn test_get_monitor_status_impl(logger: Logger) {
        ...
        let monitor_id = MonitorId::new(account_key.clone(), 10, 20, 30);
        mobilecoind_db
            .add_monitor(&monitor_id, &account_key, &data)
            .unwrap();
        ...
        mobilecoind_db.remove_monitor(&monitor_id, None).unwrap();
    ...
    fn test_get_tx_status_as_receiver_impl(logger: Logger) {
        ...
        mobilecoind_db
            .add_monitor(&monitor_id, &receiver, &data)
            .unwrap();
    ...
    fn test_get_processed_block(logger: Logger) {
        ...
        mobilecoind_db
            .add_monitor(&monitor_id, &account_key, &monitor_data)
            .unwrap();
    ...
    fn test_generate_tx(logger: Logger) {
        ...
        mobilecoind_db
            .add_monitor(&monitor_id, &sender, &data)
            .unwrap();
...

        {
            ...
            let mut request = request.clone();
            request.set_sender_monitor_id(MonitorId::new(sender, 0, 20, 0).to_vec());
            assert!(client.generate_tx(&request).is_err());
        }
    ...
    fn test_generate_optimization_tx(logger: Logger) {
        ...
        assert_eq!(
            ...
            data.account_key.clone().unwrap().subaddress(0)
        );
        assert_eq!(
        ...
        let tx_public_key = RistrettoPublic::try_from(&tx_out.public_key).unwrap();
        let account_key = data.account_key.unwrap();
        let shared_secret =
            get_tx_out_shared_secret(account_key.view_private_key(), &tx_public_key);
    ...
    fn test_add_remove_add_monitor_with_spent_key_images(logger: Logger) {
        ...
        let mut request = mc_mobilecoind_api::AddMonitorRequest::new();
        let account_key = data.account_key.clone().unwrap();
        request.set_account_key(mc_api::external::AccountKey::from(&account_key));
        request.set_first_subaddress(data.first_subaddress);
        request.set_num_subaddresses(data.num_subaddresses);
        request.set_first_block(data.first_block);
        ...
        let mut request = mc_mobilecoind_api::AddMonitorRequest::new();
        let account_key = data.account_key.clone().unwrap();
        request.set_account_key(mc_api::external::AccountKey::from(&account_key.clone()));
        request.set_first_subaddress(data.first_subaddress);
        request.set_num_subaddresses(data.num_subaddresses);
        request.set_first_block(data.first_block);
    ...
    // FIXME: add test adding monitor with password
}
