...
// we can do the symbolication. This info comes from the constructor of sgx_urts::SgxEnclave.
lazy_static! {
...
fn symbolicate_and_print_backtrace(eid: sgx_enclave_id_t, frames: &[Frame]) {
    ...
    let enclave_path = lk.get(&eid).unwrap_or_else(|| panic!("Enclave path was unknown for eid = {}, eid=0 indicates sgx_enclave_id was not initialized. Otherwise are you using sgx_urts::SgxEnclave?", eid));
    ...
    pub fn new() -> Self {
        Self(Cache::new(Box::new(|path: &PathBuf| {
            ...
            if path_cstr.as_bytes().len() <= 1 {
                eprintln!("Enclave path is not available, cannot symbolicate backtrace. Are you using sgx_urts::SgxEnclave?");
