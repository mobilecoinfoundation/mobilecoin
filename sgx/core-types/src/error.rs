...
pub enum Error {
    ...
    /// Metadata version is inconsistent between uRTS and `sgx_sign` or the uRTS
    /// is incompatible with the current platform (`0x200D`)
    ...
    /// The Key ID field is all zeroes, the encryption key cannot be regenerated
    /// (`0x7002`)
    ...
    /// A recovery file cannot be opened, so the flush operation cannot continue
    /// (`0x7005`)
