...
//! An store object for managing storage of ArchiveBlock objects in the
//! database, while taking care of de-duplicating contents when possible.
...
/// An internal object for representing ArchiveBlock that doesn't hold the
/// actual Block and BlockContents since those might be shared with other
/// blocks. The AVR is also not stored since that is typically shared with other
/// blocks.
...
/// Object for managing the storage of ArchiveBlocks.
#[derive(Clone)]
pub struct ArchiveBlockStore {
...
impl ArchiveBlockStore {
    /// Create a new ArchiveBlockStore instance.
    ...
    /// Add a single ArchiveBlock that was fetched from `src_url` into the
    /// database.
    ...
    pub fn get_block_data_map(
        ...
        for (key_bytes, value_bytes) in cursor.iter_from(&first_key_bytes).filter_map(Result::ok) {
            ...
            results.insert(
                tx_source_url,
                ArchiveBlock::new(block, block_contents, stored_block_data.signature, None),
            );
        }
