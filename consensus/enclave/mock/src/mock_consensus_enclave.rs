...
mock! {
    ...
    impl ConsensusEnclave for ConsensusEnclave {
        ...
        fn peer_connect(&self, peer_id: &ResponderId, res: PeerAuthResponse) -> ConsensusEnclaveResult<(PeerSession, EvidenceKind)>;
    ...
    impl ReportableEnclave for ConsensusEnclave {
        fn new_ereport(&self, qe_info: TargetInfo) -> SgxReportResult<(Report, EnclaveReportDataContents)>;

        fn verify_attestation_evidence(&self, attestation_evidence: DcapEvidence) -> SgxReportResult<()>;

        fn get_attestation_evidence(&self) -> SgxReportResult<DcapEvidence>;
    }
}
