...
#Setup payment invocations
payrpc1="{\"receiver_b58_address_code\": \"$add1\",\"value\": \"0.001\"}"
r1=$(curl localhost:9090/monitors/$mon_id1/subaddresses/0/pay-address-code -d "$payrpc1" -X POST -H "$enc")

#Run payments to self every 1.5 seconds. More addresses can be added above if desired
while [ true ]
do
	for var in 1
	do
