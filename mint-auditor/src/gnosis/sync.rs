    ...
    fn process_eth_transaction(&self, conn: &Conn, tx: &EthereumTransaction) -> Result<(), Error> {
        ...
        for transfer in &tx.transfers {
            ...
            if transfer.to == self.audited_safe.safe_addr {
                ...
                let token_address = transfer.token_address.clone().unwrap_or_default();
...
mod test {
    ...
    use crate::{
        db::{
            ...
            test_utils::{insert_mint_tx_from_deposit, TestDbContext},
            MintTx,
        },
        gnosis::{
