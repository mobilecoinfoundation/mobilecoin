...
use mc_fog_ledger_enclave::ENCLAVE_FILE;
...
fn main() {
    ...
    for i in 0..50 {
        let shard_uri_string = format!(
            "insecure-fog-ledger-store://node{}.test.mobilecoin.com:3225",
            i
        );
        let shard_uri = LedgerStoreUri::from_str(&shard_uri_string).unwrap();
        let fog_view_store_grpc_client = LedgerStoreApiClient::new(
    ...
    let _enclave = todo!("Fog ledger router enclave not yet implemented.");
    /*
    let mut router_server =
        LedgerRouterServer::new(config, enclave, fog_view_store_grpc_clients, logger);
    router_server.start();
    loop {
