...
impl ClientBuilder {
    ...
    fn get_fog_ingest_verifier(&self) -> Verifier {
        let mr_signer_verifier = if let Some(signature) = self.fog_ingest_sigstruct.as_ref() {
            ...
            mr_signer_verifier.allow_hardening_advisories(mc_fog_ingest_enclave_measurement::HARDENING_ADVISORIES);
            mr_signer_verifier
        } else {
    ...
    fn get_fog_ledger_verifier(&self) -> Verifier {
        let mr_signer_verifier = if let Some(signature) = self.fog_ledger_sigstruct.as_ref() {
            ...
            mr_signer_verifier.allow_hardening_advisories(mc_fog_ledger_enclave_measurement::HARDENING_ADVISORIES);
            mr_signer_verifier
        } else {
    ...
    fn get_fog_view_verifier(&self) -> Verifier {
        let mr_signer_verifier = if let Some(signature) = self.fog_view_sigstruct.as_ref() {
            ...
            mr_signer_verifier.allow_hardening_advisories(mc_fog_view_enclave_measurement::HARDENING_ADVISORIES);
            mr_signer_verifier
        } else {
