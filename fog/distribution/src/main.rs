...
fn main() {
    ...
    for (i, fog_account) in fog_accounts.iter().enumerate() {
        if let Some(new_fog_resolver) = build_and_submit_transaction(
            i,
            ...
            seed_fog_resolver = new_fog_resolver;
        }
    }
