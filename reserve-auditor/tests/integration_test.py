...
#
# Integration test that tests the reserve auditor tracks mints and burns.
...
from reserve_auditor_pb2 import *
from reserve_auditor_pb2_grpc import *
...
class ReserveAuditorClient:
    def __init__(self, address, ssl=False):
        ...
        self.stub = ReserveAuditorApiStub(self.channel)
...
class ReserveAuditorTest:
    def __init__(self, args):
        self.args = args
        self.mobilecoind_client = MobilecoindClient(args.mobilecoind_addr)
        self.reserve_auditor_client = ReserveAuditorClient(args.reserve_auditor_addr)
    ...
    def run(self):
        ...
        try:
            ...
            # Get the network block height and wait for the reserve auditor to catch up
            response = self.wait_for_reserve_auditor_to_sync()
            ...
            logging.info(f"Reserve client output: {output}")

            # Give the network time to process the mint and then for mobilecoind and the reserve auditor to catch up
            ...
            counters = self.reserve_auditor_client.get_counters()
    ...
    def wait_for_reserve_auditor_to_sync(self):
        """Wait for the reserve auditor to sync to the network block height.
        Return the last block audit data"""
        network_block_index = self.mobilecoind_client.get_network_block_index()
        for _ in range(20):
            response = self.reserve_auditor_client.get_last_block_audit_data()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--reserve-auditor-addr",
                        type=str,
                        help="reserve auditor host:port")
    parser.add_argument("--reserve-client-bin",
                        type=str,
                        help="path to the mc-consensus-reserve-client binary")
