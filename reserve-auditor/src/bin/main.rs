...
use mc_reserve_auditor::{
    db::{
        transaction, AuditedBurn, AuditedMint, BlockAuditData, BlockBalance, Conn, ReserveAuditorDb,
        SyncBlockData,
    },
...
async fn main() {
    ...
    match config.command {
        ...
        Command::StartHttpServer {
            ...
            cmd_start_http_server(reserve_auditor_db, port, host, logger).await;
        }
    }
}
...
fn cmd_scan_ledger(
    ...
    let reserve_auditor_db = ReserveAuditorDb::new_from_path(
        &reserve_auditor_db_path.into_os_string().into_string().unwrap(),
        DB_POOL_SIZE,
        logger.clone(),
    )
...
async fn cmd_start_http_server(
    ...
    start_http_server(reserve_auditor_db, port, host).await;
}
